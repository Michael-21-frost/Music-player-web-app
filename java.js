...
// Credits - https://youtu.be/OwrSOPNpPpM
      ...
      {
       singers : "Rochak Kohli, Arijit Singh, Neha Kakkar", 
...
// Credits - https://youtu.be/NJ1NIIdHhXs
    ...
    mnPlrLik.addEventListener("click",
     function(){
    ...
    for(p=1;p<playCnt.childNodes.length;p+=2){
    ...
    window.addEventListener("scroll",function(){
       if(document.body.scrollTop>="50" || window.scrollY>="50"){
    ...
    const Themes = {
       ...
       choco:["#4B839B","#3A728A","rgba(62,120,150,.4)","#6DA5BD"],
        black:["rgba(0, 0, 0, 0.699)"],
        blue:["#AD6444","#9C5333","rgba(156,83,51,.4)","#CF8666"],
        indigo:["#7D53DE","#6C42CD","rgba(125,83,222,.4)","#9F75FF"]
    }
    var root = document.querySelector(":root");
    
    function themeOpn(){
       themePlat.style.display="block";
    }
    function themeClos(){
    ...
    function setTheme(opt){
       opt = Themes[opt];
       root.style.setProperty("--theme",opt[1])
       root.style.setProperty("--theme-D",opt[0])
       root.style.setProperty("--theme-D-trans",opt[3])
       root.style.setProperty("--theme-L",opt[2])
    }
